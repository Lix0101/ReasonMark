...
class PFUtils:
    ...
    def __init__(self, config: PFConfig, *args, **kwargs) -> None:
        ...
        self.config = config
        self.pad_id = (
            config.generation_tokenizer.pad_token_id
            if config.generation_tokenizer.pad_token_id is not None
            else config.generation_tokenizer.eos_token_id
        )
    ...
    def score_tok(self, ngram_tokens: List[int], token_id: int) -> torch.Tensor:
        ...
        self.rng.manual_seed(seed)
        rs = torch.rand(
            self.config.vocab_size, generator=self.rng, device=self.rng.device
        )
        # avoid log(0)
        rs[rs == 0] = 1e-4
    ...
    def get_scores_by_t(
        ...
        """
        tokens_id = self.config.generation_tokenizer.encode(
...
class PF(BaseWatermark):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(algorithm_config, str):
            ...
            raise TypeError(
                "algorithm_config must be either a path string or a SIRConfig instance"
            )
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        prompt_tokens = self.config.generation_tokenizer.encode(
            prompt, add_special_tokens=False
        )
        min_prompt_size = len(prompt_tokens)
        total_len = min(
            self.config.max_seq_len,
            self.config.gen_kwargs["max_new_tokens"] + min_prompt_size,
        )

        tokens = (
            torch.full((total_len,), self.utils.pad_id)
            .to(self.config.generation_model.device)
            .long()
        )
        ...
        for cur_pos in range(start_pos, total_len):
            outputs = self.config.generation_model.forward(
                tokens[prev_pos:cur_pos].unsqueeze(0),
                use_cache=True,
                past_key_values=(
            ...
            next_toks = self.utils.sample_next(
        ...
        tokens = tokens[
            : len(prompt_tokens) + self.config.gen_kwargs["max_new_tokens"]
        ]
