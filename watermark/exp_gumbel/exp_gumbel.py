...
class EXPGumbelUtils:
    ...
    def __init__(self, config: EXPGumbelConfig, *args, **kwargs) -> None:
        ...
        self.uniform = torch.clamp(
            torch.rand(
                (
                    self.config.vocab_size * self.config.prefix_length,
        ...
        self.gumbel = (
            -torch.log(
                torch.clamp(-torch.log(self.uniform), min=self.config.eps)
            )
...
class EXPGumbelLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        self, config: EXPGumbelConfig, utils: EXPGumbelUtils, *args, **kwargs
    ) -> None:
        """
    ...
    def __call__(
        ...
        if input_ids.shape[-1] < self.config.prefix_length:
            return scores
        prev_token = torch.sum(
            input_ids[:, -self.config.prefix_length :], dim=-1
        )  # (batch_size,)
        gumbel = self.utils.gumbel[prev_token]  # (batch_size, vocab_size)
        return (
            scores[..., : gumbel.shape[-1]] / self.config.temperature + gumbel
        )
...
class EXPGumbel(BaseWatermark):
    ...
    def __init__(
        self,
        algorithm_config: str | EXPGumbelConfig,
        ...
        **kwargs,
        ...
        if isinstance(algorithm_config, str):
            ...
            raise TypeError(
                "algorithm_config must be either a path string or a EXPGumbelConfig instance"
            )

        self.utils = EXPGumbelUtils(self.config)
        self.logits_processor = EXPGumbelLogitsProcessor(
        ...
        for i in range(self.config.sequence_length):
            with torch.no_grad():
                if past:
                    ...
                    output_gumbel = self.logits_processor(
                        input_ids=inputs, scores=output.logits
                    )
                else:
    ...
    def detect_watermark(
        ...
        for i in range(self.config.prefix_length, seq_len):
            prev_tokens_sum = torch.sum(
                encoded_text[i - self.config.prefix_length : i], dim=-1
            )
            token = encoded_text[i]
            u = self.utils.uniform[prev_tokens_sum, token]
            score += log(1 / (1 - u))

        p_value = scipy.stats.gamma.sf(
            score, seq_len - self.config.prefix_length, loc=0, scale=1
        )
    ...
    def get_data_for_visualization(
        ...
        decoded_tokens = [
            self.config.generation_tokenizer.decode([token_id])
