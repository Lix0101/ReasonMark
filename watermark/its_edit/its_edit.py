...
class ITSEdit(BaseWatermark):
    ...
    def generate(
        ...
        for i in range(m):
            ...
            tokens = sampler(
                probs,
                pis,
                xis[torch.arange(batch_size), (offset.squeeze() + i) % n],
            ).to(model.device)
    ...
    def generate_unwatermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        unwatermarked_prompt = generate_rnd(
            encoded_prompt.unsqueeze(0),
            self.config.sequence_length,
            self.config.generation_model,
        )[0, prompt_tokens:]
        return self.config.generation_tokenizer.decode(
            unwatermarked_prompt, skip_special_tokens=True
        )
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs):
        ...
        prompt_tokens = len(encoded_prompt)
        generate_watermark = lambda prompt, seed: self.generate(
            self.config.generation_model,
            prompt,
        ...
        watermark_sample = generate_watermark(
            encoded_prompt.unsqueeze(0), seeds[0].unsqueeze(0)
        )[0, prompt_tokens:]
    ...
    def detect_watermark(
        ...
        test_stat = lambda tokens, n, k, generator, vocab_size, null=False: self.utils.phi(
            tokens=tokens,
            n=n,
            k=k,
            generator=generator,
        ...
        test = lambda tokens, seed: self.permutation_test(
            tokens,
            self.config.vocab_size,
            self.config.pseudo_length,
            len(tokens),
        ...
        if return_dict:
            return {
                "is_watermarked": (
                    pval_watermark <= self.config.p_threshold
                ).item(),
                "score": pval_watermark.item(),
            }
        else:
            return (
                (pval_watermark <= self.config.p_threshold).item(),
                pval_watermark.item(),
            )
