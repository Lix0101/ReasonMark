...
class EXPUtils:
    ...
    def seed_rng(self, input_ids: torch.Tensor) -> None:
...
class EXP(BaseWatermark):
    ...
    def __init__(
        self,
        algorithm_config: str | EXPConfig,
        ...
        **kwargs,
        ...
        if isinstance(algorithm_config, str):
            ...
            raise TypeError(
                "algorithm_config must be either a path string or a EXPConfig instance"
            )
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        for i in range(self.config.sequence_length):
            ...
            probs = torch.nn.functional.softmax(
                output.logits[:, -1, : self.config.vocab_size]
            ...
            random_numbers = torch.rand(
                self.config.vocab_size, generator=self.utils.rng
            )

            # Sample token to add watermark
            token = self.utils.exp_sampling(probs, random_numbers).to(
                self.config.device
            )
    ...
    def generate_watermarked_text_rllm(
        ...
        for _ in range(self.config.sequence_length):
            ...
            if passed_think:
                # 对于回答部分，使用水印采样
                # 根据前缀生成随机数，只使用 </think> 之后的部分
                context_for_seed = inputs[0, think_end_pos + 1 :]
                # 如果前缀长度不够，利用起始输入的 prompt
                if len(context_for_seed) < self.config.prefix_length:
                    context_for_seed = torch.cat(
                        [
                            inputs[0, : self.config.prefix_length],
                            context_for_seed,
                        ],
                        dim=0,
                    )
                self.utils.seed_rng(context_for_seed.cpu())
                random_numbers = torch.rand(
                    self.config.vocab_size, generator=self.utils.rng
                )
