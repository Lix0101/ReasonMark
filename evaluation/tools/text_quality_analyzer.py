...
class TextQualityAnalyzer:
    ...
    def analyze(self, text: str, *args, **kwargs):
        pass
...
class ReferencedTextQualityAnalyzer(TextQualityAnalyzer):
    ...
    def analyze(self, text: str, reference, *args, **kwargs):
        pass
...
class ExternalDiscriminatorTextQualityAnalyzer(TextQualityAnalyzer):
    ...
    def analyze(
        self, text1: str, text2: str, description: str, *args, **kwargs
    ):
        pass
...
class PPLCalculator(DirectTextQualityAnalyzer):
    ...
    def analyze(self, text: str, *args, **kwargs):
        ...
        logits = self.model(
            torch.unsqueeze(encoded_text, 0), return_dict=True
        ).logits[0]
...
class PrefixPPLCalculator(DirectTextQualityAnalyzer):
    """Perplexity calculator with prefix handling for text quality analysis."""

    def __init__(self, model, tokenizer, device="cuda") -> None:
        """
        Initialize the prefix perplexity calculator.
    ...
    def analyze(self, text: str, prefix: str = "", *args, **kwargs):
        """
        Calculate the perplexity of the given text, ignoring the prefix in loss calculation.

        Parameters:
            text (str): The text to calculate perplexity for.
            prefix (str): The prefix that should not be included in loss calculation.
...
class LogDiversityAnalyzer(DirectTextQualityAnalyzer):
    ...
    def analyze(self, text: str, *args, **kwargs):
        ...
        ngram_results, unique_tokens = self._eval_one_instance(
            stripped_text, ngram_list
        )
        ...
        diversity_scores = [
            1
            - (prediction_results[n]["unique"] / prediction_results[n]["total"])
            for n in ngram_list
        ]

        # Overall diversity is the product of individual n-gram diversities
        overall_diversity = (
            (1 - diversity_scores[0] / 100)
            * (1 - diversity_scores[1] / 100)
...
class BLEUCalculator(ReferencedTextQualityAnalyzer):
    ...
    def analyze(self, text: str, reference: str, *args, **kwargs):
...
class PassOrNotJudger(ReferencedTextQualityAnalyzer):
    ...
    def _check_correctness(
        self, prompt: str, completion: str, test: str, entry_point: str
    ):
    ...
    def analyze(self, text: str, reference: dict, *args, **kwargs):
        """Check if the text passes the correctness test."""
        passed = self._check_correctness(
            reference["task"], text, reference["test"], reference["entry_point"]
        )
        return passed
...
class GPTTextDiscriminator(ExternalDiscriminatorTextQualityAnalyzer):
    ...
    def analyze(self, text1: str, text2: str, question: str, *args, **kwargs):
...
class MathAccuracyCalculator(ReferencedTextQualityAnalyzer):
    """数学推理任务正确率计算器"""
    ...
    def analyze(
        self, text: str, reference: str, *args, **kwargs
    ) -> Literal[1, 0]:
        """
        通过完全匹配计算数学推理任务的正确率
        ...
        返回:
            Literal[1, 0]: 如果完全匹配返回 1，否则返回 0
        """
        return 1 if text.strip() == reference.strip() else 0
