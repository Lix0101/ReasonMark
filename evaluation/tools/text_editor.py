...
class RandomWalkAttack(TextEditor):
    ...
    def __init__(
        self,
        perturbation_tokenizer: T5Tokenizer,
        perturbation_oracle: T5ForConditionalGeneration,
        quality_oracle: QualityOracle,
        device="cuda",
        total_steps=200,
        span_len=6,
        target_valid_steps=100,
        **kwargs,
    ):
        """
        Parameters:
    ...
    def perturb(self, text: str):
        ...
        # Tokenize the input
        final_input = self.perturbation_tokenizer(
        ...
        with torch.inference_mode():
            outputs = self.perturbation_oracle.generate(
                **final_input, **self.gen_kwargs
            )
        outputs = self.perturbation_tokenizer.batch_decode(
            outputs, skip_special_tokens=True
        )

        infilled_text = outputs[0]
        final_output_text = final_input_text.replace(
            "<extra_id_0>", infilled_text
        )
    ...
    def edit(
        ...
        while (
            n_iter < self.total_steps or valid_steps < self.target_valid_steps
        ):
            candidate_response = self.perturb(n_response)

            candidate_response = self.grammatical_error_correction(
                candidate_response
            )
            candidate_response = self.remove_incomplete_sentences(
                candidate_response
            )

            if self.quality_oracle.maintain_quality(
                prompt, original_response, candidate_response
            ):
                ...
                print(
                    f"Get a better {valid_steps}-th response at step {n_iter}/{self.total_steps}: {n_response}"
                )
                patience = 0
            else:
                patience += 1
    ...
    def mask_text(self, text):
        ...
        masked_text = (
            " ".join(words[:start]) + " <extra_id_0> " + " ".join(words[end:])
        )
        return masked_text
    ...
    def remove_incomplete_sentences(self, text):
        ...
        for sent in sentences:
            if (
                sent.endswith(".")
                and not self.contains_verb(sent)
                and not bool(re.match(r"^\d+\.$", sent))
            ):
                continue
            else:
    ...
    def edit(self, text: str, reference: str):
        ...
        for sent_idx in range(0, len(sentences), self.sent_interval):
            curr_sent_window = " ".join(
                sentences[sent_idx : sent_idx + self.sent_interval]
            )
...
class WordDeletion(TextEditor):
    ...
    def edit(self, text: str, reference=None):
        ...
        edited_words = [
            word for word in word_list if random.random() >= self.ratio
        ]
...
class WordInsertion(TextEditor):
    ...
    def __init__(
        ...
        ----------
        ratio : float
            Proportion of the original words that will be inserted again.
        attack_type : {"dispersed", "local"}
            dispersed – insert many words at random positions;
            local     – insert a single word at one random position.
        word_list : list[str] | None
            Candidate words to insert. If None, try NLTK `words`; fall back
            to a small built-in list when corpus is absent.
        """
        self.ratio = ratio
        ...
        if word_list is not None:
            ...
            try:
                ...
                self.word_list = nltk_words.words()
            except LookupError:
                # Fallback small list to avoid online download
                print("Fallback small list to avoid online download")
                self.word_list = [
                    "the",
                    "and",
                    "a",
    ...
    def edit(self, text: str, reference=None):
        ...
        words = text.split()
        n_insert = max(1, int(len(words) * self.ratio))

        if self.attack_type == "local":
            pos = np.random.randint(0, len(words) + 1)
            words.insert(pos, np.random.choice(self.word_list))
        else:  # dispersed
...
class SynonymSubstitution(TextEditor):
    ...
    def __init__(self, ratio: float) -> None:
        ...
        nltk.download('words', download_dir='/root/markllm/MarkLLM-dev/outputs/attack_results')
    ...
    def edit(self, text: str, reference=None):
        ...
        num_to_replace = min(
            int(self.ratio * num_words), len(replaceable_indices)
        )

        # Randomly select words to replace
        if num_to_replace > 0:
            indices_to_replace = random.sample(
                replaceable_indices, num_to_replace
            )
            ...
            for i in indices_to_replace:
                ...
                new_word = (
                    random.choice(chosen_syn.lemmas()[1:])
                    .name()
                    .replace("_", " ")
                )
                words[i] = new_word
...
class MathReasoningTextEditor(TextEditor):
    """提取数学推理任务中 \\boxed{} 内的最终答案"""
    ...
    def edit(self, text: str, reference=None):
        ...
        参数:
            text (str): 包含 \\boxed{} 的数学推理文本
            reference (str, optional): 参考文本，本方法不使用
...
class CodeGenerationV2TextEditor(TextEditor):
    """从 Markdown 代码块中提取代码"""
    ...
    def __init__(self, language: str = "python") -> None:
        ...
        参数:
            language (str): 代码块的语言标识，默认为 "python"
        """
        super().__init__()
...
class DeepSeekParaphraser(TextEditor):
    """
    Paraphrase a text via DeepSeek online API (OpenAI-compatible).
    ...
    def __init__(
        ...
        self.system_prompt = (
            ...
            "When given some text, you MUST output only the paraphrased text, "
            "without any additional commentary, labels, or surrounding quotation marks."
        )

    def edit(self, text: str, reference=None):
        """Return a meaning-preserving paraphrase produced by DeepSeek."""
        user_prompt = (
            "Paraphrase the text below while preserving its meaning. "
            "Return ONLY the paraphrased text, with no extra words or labels.\n\n"
            f"{text}"
        )
...
class DeepSeekBackTranslationTextEditor(TextEditor):
    """
    Back-translation attack powered by DeepSeek API (English → Chinese → English).
    ...
    def _translate(self, text: str, source_lang: str, target_lang: str) -> str:
        """Translate `text` from `source_lang` to `target_lang` via DeepSeek."""
        system_prompt = (
            f"You are a helpful assistant that translates text from "
            f"{source_lang} to {target_lang}. "
            "When given some text, you MUST output only the translated text, "
            "without any additional commentary, labels, or surrounding quotation marks."
        )
        response = self.client.chat.completions.create(
            model=self.model,
    ...
    def edit(self, text: str, reference=None):
        """Perform English → Chinese → English back-translation."""
        chinese_text  = self._translate(text,        "English", "Chinese")
        english_text  = self._translate(chinese_text,"Chinese", "English")
        return english_text